...
[prompts/staff-engineer.md](prompts/staff-engineer.md) is the full worldview as a system prompt. Each task prompt carries the slice of it that task needs, so every prompt also works standalone.
...
- **Each task prompt works as a single user message.** `prompts/staff-engineer.md` is the exception — it is designed as a system prompt.
- **Stacking.** In tools with a system prompt slot: system = `staff-engineer.md`, user = the filled-in task prompt. In a plain chat: just the task prompt. The overlap between them is deliberate and harmless — repetition of the core rules costs nothing on strong models and helps weak ones.
